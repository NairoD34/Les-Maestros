class AutoCompleteAddress {
  ...
  constructor() {
    this.inputCity = document.getElementById(
      "ville"
    ) as HTMLInputElement | null;
    this.inputId = document.getElementById(
      "villeId"
    ) as HTMLInputElement | null;
    this.inputCpId = document.getElementById(
      "selectedPostalCodesId"
    ) as HTMLInputElement | null;
    this.dataCities = document.getElementById(
      "cityList"
    ) as HTMLDataListElement | null;
    this.codePostauxSelect = document.getElementById(
      "postalCodeSelect"
  ...
  private initializeEventListeners(): void {
    if (this.inputCity) {
      ...
      this.inputCity.addEventListener("input", (event: Event) => {
        if (debounceTimeout) {
  ...
  private addCitySelectionListener(): void {
    if (this.inputCity) {
      this.inputCity.addEventListener("input", (event: Event) =>
        this.handleCitySelection(event)
      );
    }
  }
  ...
  private handleCitySelection(event: Event): void {
    const target = event.target as HTMLInputElement;
    const selectedOption = this.dataCities?.querySelector(
  ...
  private updatePostalCodeSelect(
    codesPostaux: { libelle: string; id: string }[]
  ): void {
  ...
  private addPostalCodeChangeListener(
    codesPostaux: { libelle: string; id: string }[]
  ): void {
    if (this.codePostauxSelect) {
      this.codePostauxSelect.addEventListener("change", () => {
        ...
        if (this.inputCpId) {
          this.inputCpId.value = selectedPostalCode
            ? selectedPostalCode.id
            : "";
        }
      });
    }
